...
if (isset($_GET['query'])) {
    ...
    if (!empty($search_query)) {
        ...
        foreach ($superheroes as $hero) {
            if (
                strtolower($hero['alias']) === $search_term ||
                strtolower($hero['name']) === $search_term
            ) {
        ...
        if ($found_superhero) {
            echo "<h3>" . strtoupper(htmlspecialchars($found_superhero['alias'])) . "</h3>";
            echo "<h4>A.K.A " . htmlspecialchars($found_superhero['name']) . "</h4>";
            echo "<p>" . htmlspecialchars($found_superhero['biography']) . "</p>";
            exit; 
        } else {
